...
int RAK_TcpServer(uint16_t lPort)//返回调用监听函数之后的tcp套接字
{
    SOCKADDR_IN     LocalAddr;
    static int      tmp_sockfd = INVAILD_SOCK_FD;    
    int             sockfd;
    ...
    LocalAddr.sin_family = AF_INET;//目前仅支持AF_INET地址簇
    LocalAddr.sin_port = htons((uint16_t)lPort);
    LocalAddr.sin_addr = 0;
    ...
    ret = listen(tmp_sockfd, 1);//第二个参数未使用 填写1即可 listen函数是用来侦听最大请求数 我觉得这里的1是表示最大侦听数为1
    if (ret == RW_ERR || ret == RW_ERR_TIME_OUT || ret ==RW_ERR_SOCKET_INVAILD)
    {
      close(tmp_sockfd);
      return ret;
    ...
    {
      sockfd = tmp_sockfd;
      tmp_sockfd = INVAILD_SOCK_FD;//此变量为静态变量
      return sockfd;
    }
...
int RAK_TcpSRecvClients(int sever_fd, RW_APP_CTX* app_ctx)
{
    ...
    ret = select(sever_fd, 1);//等待1ms监听套接字为sever_fd的数据
    if (ret == RW_ERR || ret == RW_ERR_TIME_OUT) 
    {
        return ret;
    }
    ...
    if (sockfd > 0 )
    {
      ...
      {
        if(app_ctx->ltcps_clientinfos[i].tcpc_sockfd == -1)//-1表示RW_ERR 
        {
...
void RAK_TcpServer_EventHandle(void)
{
    ...
    if (app_demo_ctx.ltcps_sockfd == INVAILD_SOCK_FD)//初始化的时候就定义的是app_demo_ctx.ltcps_sockfd == INVAILD_SOCK_FD
    {
        if((ret =RAK_TcpServer(25000)) >= 0)//返回的就是套接字 参数为lPort 会调用htons转化
        {
    ...
    RAK_TcpSRecvClients(app_demo_ctx.ltcps_sockfd , &app_demo_ctx);//调用accept函数更新套接字
    ...
    if (app_demo_ctx.tcpc_num > 0)
    {
      for (i=0; i< ALLOW_MAX_NUMS; i++)
      {
        if (app_demo_ctx.ltcps_clientinfos[i].tcpc_sockfd ==INVAILD_SOCK_FD)
          ...
          if (ret <= 0 )//返回值为接收的字节数，所以小于0的表示有错误
          {
            if (ret ==RW_ERR_SOCKET_INVAILD){
